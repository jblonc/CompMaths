    ...
    void participate_save(int j){
         ...
         OdbcCommand updComm=new OdbcCommand("UPDATE participants SET name=? WHERE student_id=?",dbconn); // SET student_id=?, name=? WHERE student_id=?
    ...
    void participate_confirm(int j){
         ...
         if(countRead.Read()){int shortOfT=12-Convert.ToInt32(countRead.GetInt32(0)); countRead.Close();// int is treated as int32; GetInt16 doesn't work
                             if(shortOfT!=0){
